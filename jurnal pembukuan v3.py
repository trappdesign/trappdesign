...
# Fungsi untuk menambahkan stock barang
def tambah_stock():
    nama_barang = entry_nama_barang_stock.get()
    jumlah = entry_jumlah_stock.get()

    if not nama_barang or not jumlah:
        messagebox.showwarning("Input Kosong", "Harap lengkapi semua field!")
        return

    # Tambahkan data stock ke list
    data_stock.append([nama_barang, jumlah])
    update_tabel_stock()

    # Kosongkan field setelah menambahkan data
    clear_field_stock()

# Fungsi untuk mengosongkan field stock
def clear_field_stock():
    entry_nama_barang_stock.delete(0, tk.END)
    entry_jumlah_stock.delete(0, tk.END)

# Fungsi untuk memperbarui tabel stock
def update_tabel_stock():
    for row in tabel_stock.get_children():
        tabel_stock.delete(row)
    for data in data_stock:
        tabel_stock.insert("", "end", values=data)
...
# Tab Penjualan
penjualan_frame = ttk.Frame(notebook)
notebook.add(penjualan_frame, text="Penjualan")

# Frame untuk input data penjualan
input_frame = tk.LabelFrame(penjualan_frame, text="Input Data Penjualan", padx=10, pady=10)
...
entry_total_harga = tk.Entry(input_frame, width=30, state="readonly")
...
laporan_penjualan_frame = tk.LabelFrame(penjualan_frame, text="Laporan Penjualan", padx=10, pady=10)
...
# Tab Data Pelanggan
pelanggan_frame = ttk.Frame(notebook)
notebook.add(pelanggan_frame, text="Data Pelanggan")

# Frame untuk form data pelanggan
form_pelanggan_frame = tk.LabelFrame(pelanggan_frame, text="Form Data Pelanggan", padx=10, pady=10)
...
tabel_pelanggan_frame = tk.LabelFrame(pelanggan_frame, text="Data Pelanggan", padx=10, pady=10)
tabel_pelanggan_frame.pack(padx=10, pady=10, fill="both", expand=True)
...
notebook.add(stock_frame, text="Stock Barang")

# Frame untuk form stock barang
form_stock_frame = tk.LabelFrame(stock_frame, text="Form Stock Barang", padx=10, pady=10)
form_stock_frame.pack(padx=10, pady=10, fill="x")

# Label dan Entry untuk Nama Barang
tk.Label(form_stock_frame, text="Nama Barang:").grid(row=0, column=0, sticky="w", pady=5)
entry_nama_barang_stock = tk.Entry(form_stock_frame, width=30)
entry_nama_barang_stock.grid(row=0, column=1, pady=5)

# Label dan Entry untuk Jumlah Stock
tk.Label(form_stock_frame, text="Jumlah Stock:").grid(row=1, column=0, sticky="w", pady=5)
entry_jumlah_stock = tk.Entry(form_stock_frame, width=30)
entry_jumlah_stock.grid(row=1, column=1, pady=5)

# Tombol Tambah Stock
button_tambah_stock = tk.Button(form_stock_frame, text="Tambah Stock", command=tambah_stock)
button_tambah_stock.grid(row=2, column=1, pady=10)

# Frame untuk tabel stock barang
tabel_stock_frame = tk.LabelFrame(stock_frame, text="Data Stock Barang", padx=10, pady=10)
tabel_stock_frame.pack(padx=10, pady=10, fill="both", expand=True)

# Tabel untuk menampilkan stock barang
columns_stock = ("Nama Barang", "Jumlah Stock")
tabel_stock = ttk.Treeview(tabel_stock_frame, columns=columns_stock, show="headings")
tabel_stock.heading("Nama Barang", text="Nama Barang")
tabel_stock.heading("Jumlah Stock", text="Jumlah Stock")
tabel_stock.pack(fill="both", expand=True)
